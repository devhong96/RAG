...
- [1. 전체 RAG 파이프라인 데이터 흐름](#1-전체-rag-파이프라인-데이터-흐름)
- [2. `src/lib/` 단계별 상세 역할](#2-srclib-단계별-상세-역할)
- [3. 외부 폴더와 `lib`의 관계](#3-외부-폴더와-lib의-관계)
- [4. 아키텍처 핵심 규칙 요약](#4-아키텍처-핵심-규칙-요약)
...
[문서 원본]
   ...
   ▼ 3단계: 똑똑하게 검색하기 (검색 전략)
   src/lib/search/
     • hybrid.ts       : 문자열 포함 검색 + 벡터 검색 결합, RRF 순위 융합
